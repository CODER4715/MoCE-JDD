...
class DepthwiseSeparableConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, stride=1):
        ...
        self.bn = nn.BatchNorm2d(out_channels)  # 添加 BatchNorm2d
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        x = self.depthwise(x)
        x = self.pointwise(x)
        x = self.bn(x)  # 应用 BatchNorm2d
        x = self.relu(x)
        return x


class MinDownBlock(nn.Module):
    '''精简的下采样块：深度可分离卷积(stride=2)'''

    def __init__(self, in_ch, out_ch):
        super(MinDownBlock, self).__init__()
        self.down = DepthwiseSeparableConvBlock(in_ch, out_ch, kernel_size=3, padding=1, stride=2)

    def forward(self, x):
        return self.down(x)


class MinUpBlock(nn.Module):
    '''精简的上采样块：Conv2d => PixelShuffle(2) => BN => ReLU'''

    def __init__(self, in_ch, out_ch):
        super(MinUpBlock, self).__init__()
        # 使用卷积将通道数调整为 (out_ch * scale * scale)
        # 这里的 scale = 2，所以是 out_ch * 4
        self.up_conv = DepthwiseSeparableConvBlock(in_ch, out_ch * 4, kernel_size=3, padding=1, stride=1)
        self.pixel_shuffle = nn.PixelShuffle(2)  # 2 倍上采样
        self.bn = nn.BatchNorm2d(out_ch)  # PixelShuffle 后通道数变为 out_ch
        self.relu = nn.ReLU(inplace=True)
...
class MinDenBlock(nn.Module):
    """
    FastDVDnet的极其精简版去噪块，使用深度可分离卷积和BN层，保留类U-Net结构。
    上采样层使用卷积后接PixelShuffle(2)。
    旨在大幅度减少FLOPs并保持训练稳定性。
    """

    def __init__(self, num_input_frames=3):
        super(MinDenBlock, self).__init__()
        self.chs_enc0 = 16
        self.chs_enc1 = 48
        self.chs_enc2 = 96
        self.chs_dec0 = 48  # 对应 chs_enc1
        self.chs_dec1 = 16  # 对应 chs_enc0

        # 编码器部分
        self.inc = DepthwiseSeparableConvBlock(num_input_frames * 3, self.chs_enc0)
        self.down0 = MinDownBlock(self.chs_enc0, self.chs_enc1)
        self.down1 = MinDownBlock(self.chs_enc1, self.chs_enc2)

        # 解码器部分
        self.up0 = MinUpBlock(self.chs_enc2, self.chs_dec0)
        self.up1 = MinUpBlock(self.chs_dec0, self.chs_dec1)

        self.outc= nn.Conv2d(self.chs_dec1, 3, kernel_size=3, padding=1, bias=False)



    def forward(self, in0, in1, in2):
        '''Args:
           inX: Tensor, [N, C, H, W] 范围在 [0., 1.] 之间
        '''
        x = torch.cat((in0, in1, in2), dim=1)

        # 编码器路径
        enc0 = self.inc(x)
        enc1 = self.down0(enc0)
        enc2 = self.down1(enc1)

        # 解码器路径
        dec0 = self.up0(enc2) + enc1  # 新增上采样，并跳跃连接到 enc1
        dec1 = self.up1(dec0) + enc0  # 调整 up1 的输入和跳跃连接到 enc0

        output_features = dec1

        # 最终输出
        estimated_noise = self.outc(output_features)

        # 残差连接
        out = in1 - estimated_noise
...
class MoCEJDD_ISP(nn.Module):
    def __init__(self,
                 ...
                 rank_type="constant",
                 topk=1,
                 expert_layer=SelfAttention,  # SelfAttention/FFTAttention
                 with_complexity=False,
                 complexity_scale="max",
                 need_isp=False,
                 ):
        super(MoCEJDD_ISP, self).__init__()

        # self.stage1 = InputCvBlock(num_in_frames=3, out_ch=3)

        self.stage1 = MinDenBlock(num_input_frames=3)
        ...
        self.outconv = nn.Conv2d(out_channels, 3, kernel_size=7, stride=1, padding=3, bias=bias)
    ...
    def forward(self, x, metadata=None):
        ...
        # x20 = self.stage1(torch.cat((x0, x1, x2), dim=1))
        # x21 = self.stage1(torch.cat((x1, x2, x3), dim=1))
        # x22 = self.stage1(torch.cat((x2, x3, x4), dim=1))

        x20 = self.stage1(x0, x1, x2)
        x21 = self.stage1(x1, x2, x3)
        x22 = self.stage1(x2, x3, x4)

        x = torch.cat((x20, x21, x22, x2), dim=1)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        random.seed(seed)
        # opt = train_options()
        model = MoCEJDD_ISP(rank=2, num_blocks=[2, 4, 6, 8], num_dec_blocks=[2, 3, 4], levels=4, dim=8,
                        num_refinement_blocks=4, with_complexity=True, complexity_scale="max",
                        stage_depth=[1, 1, 1], heads=[2, 2, 4, 8], depth_type="constant", rank_type="spread",
                        num_experts=4, topk=1).cuda()
        # print(model)
        # import os
        # checkpoint_path = os.path.join(r'H:\My-JDD\checkpoints\exp003_netdepcv_L1_fft_vgg_208_selfattn', "last.ckpt")
        # print(f"Loading checkpoint from: {checkpoint_path}")
        # checkpoint = torch.load(checkpoint_path, map_location='cpu')
        # model_state_dict = {k.replace('net.', ''): v for k, v in checkpoint['state_dict'].items() if
        #                     k.startswith('net.')}
        ...
        flops, macs, params = calculate_flops(model=model,
                                              input_shape=(1, 5, 3, 2160, 3840),  # (1, 5, 3, 2160, 3840)
