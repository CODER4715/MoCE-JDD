...
from data.datasets_v0 import REDS_Train_Dataset, REDS_Val_Dataset
...
class PLTrainModel(pl.LightningModule):
    def __init__(self, opt):
        ...
        self.ham_demosaic = HamiltonAdam(pattern='rggb').cuda()

        self.net = FastDVDnet(need_isp=opt.isp_bwd,)
    ...
    def configure_optimizers(self):
        ...
        if self.opt.fine_tune_from:
            if self.trainer.resume_from_checkpoint:
                ckpt = torch.load(self.trainer.resume_from_checkpoint)
